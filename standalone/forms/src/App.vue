<template>
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                <h1>Add Blog Post</h1>

                <!-- Input box -->
                <form @submit.prevent="isSubmited = true">
                    <div class="form-group">
                        <label for="title">Title: </label>
                        <input type="text" class="form-control" name="" id="title" v-model.lazy.trim="post.title">
                    </div>

                <!-- Text Area -->
                     <div class="form-group">
                        <label for="content">Content: </label>
                        <!-- Modifier: Trim modifier - Removes the beginning and ending space -->
                        <textarea class="form-control" id="content" cols="30" rows="10" v-model.trim="post.content"></textarea>
                    </div>

                <!-- Checkbox -->
                     <div class="form-group">
                        <label for="publish-immediately">Publish Immediately: </label>
                        <input type="checkbox" id="publish-immediately" v-model="post.publishImmediately">
                    </div>

                    <!-- Add a checkbox for each iteration -->
                    <!-- Share on multiple social media sites -->
                    <div class="form-group">
                        <label for="">Share on</label>
                        <div v-for="media in formData.socialMedia">
                            <input type="checkbox" :id="media" :value="media" v-model="post.shareOn">
                            <label :for="media">{{ media }}</label>
                        </div>
                    </div>

                    <!-- Select element -->
                    <div class="form-group">
                        <select class="form-control" id="select-series" v-model="post.series">
                            <!-- Default value in select element -->
                            <option value="">Choose series</option>
                            <option v-for="series in formData.series" :value="series">{{ series }}</option>
                        </select>
                    </div>

                    <input type="submit" class="btn btn-primary" value="Publish">
                </form>

                <hr>

                <!-- Start table -->
                <table class="table table-striped" v-if="isSubmited">
                    <thead>
                        <tr>
                            <td class="col-xs-6"><strong>Field</strong></td>
                            <td class="col-xs-6"><strong>Value</strong></td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>Title</td>
                            <td>{{ post.title }}</td>
                        </tr>
                        <tr>
                            <td>Content</td>
                            <td style="white-space: pre;">{{ post.content }}</td>
                        </tr>
                        <tr>
                            <td>Publish immediately</td>
                            <td>{{ post.publishImmediately }}</td>
                        </tr>
                        <tr>
                            <td>Share on</td>
                            <td>
                                <ul>
                                    <li v-for="media in post.shareOn">{{ media }}</li>
                                </ul>
                            </td>
                        </tr>
                        <tr>
                            <td>Category</td>
                            <td>{{ post.category }}</td>
                        </tr>
                        <tr>
                            <td>Series</td>
                            <td>{{ post.series }}</td>
                        </tr>
                    </tbody>
                </table>
                <!-- End table -->
            </div>
        </div>
    </div>
</template>

<script>
export default {
  data() {
    return {
      isSubmited: false,
      post: {
        title: "",
        content: "",
        publishImmediately: false,
        shareOn: [],
        category: "",
        series: ""
      },
      formData: {
        socialMedia: ["Facebook", "Twitter"],
        categories: ["Frontend", "Backend"],
        series: [
          "Vue.js: From Beginner to Professional",
          "Complete Guide to Elasticsearch"
        ]
      }
    };
  }
};
</script>

<style>
input[type="radio"] + label,
input[type="checkbox"] + label {
  font-weight: normal;
}
</style>
